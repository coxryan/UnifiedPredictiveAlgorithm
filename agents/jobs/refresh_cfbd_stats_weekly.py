...
from agents.collect import CfbdClients, ApiCache, build_team_stat_features
...
def main() -> None:
    ...
    # Backfill into team inputs if they already exist
    team_inputs = read_dataset("upa_team_inputs_datadriven_v0")
    if not team_inputs.empty and "team" in team_inputs.columns:
        enriched = team_inputs.merge(features, on="team", how="left")
        enriched["season"] = year
        delete_rows("upa_team_inputs_datadriven_v0", "season", year)
        storage_write_dataset(enriched, "upa_team_inputs_datadriven_v0", if_exists="append")
